...
class LoginForm(forms.Form):
    username_or_email = forms.CharField(label='用户名或邮箱', widget=forms.TextInput(attrs={'class':'form-control','placeholder':'用户名或邮箱'}))
    password = forms.CharField(label='密码', widget=forms.PasswordInput(attrs={'class':'form-control','placeholder':'密码'}))

    #验证登录,返回给Views.py
    def clean(self):
        #这里的self.cleaned_data['username_or_email']     *************************
        #应该用self.cleand_data.get('username_or_email', '')可能会更好************
        #  用下面则需要在前端判断是否为空字段，否则报错
...
class BindEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.request.user.email != '':
            raise forms.ValidationError('用户已注册过邮箱')
...
class ChangePassword(forms.Form):
    ...
    new_password_again= forms.CharField(label='请再输入新密码', 
        min_length=6,
        widget=forms.PasswordInput(attrs={'class':'form-control','placeholder':'重复新密码'}))
